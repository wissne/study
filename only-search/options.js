...
function translateXML(xmlnode){
	var translate = "<strong>≤È—Ø:</strong><br/>";
...
function mainFrameQuery(){
	...
	if (noBaseTrans == false) {
		if(langType=='ko')
			basetrans = "<strong>∫´∫∫∑≠“Î:</strong><br/>" + basetrans;
		else if (langType == 'jap')
			basetrans = "<strong>»’∫∫∑≠“Î:</strong><br/>" + basetrans;
		else if (langType == 'fr')
			basetrans = "<strong>∑®∫∫∑≠“Î:</strong><br/>" + basetrans;
		else basetrans = "<strong>”¢∫∫∑≠“Î:</strong><br/>" + basetrans;
    	res.innerHTML = basetrans;
	}
	if (noWebTrans == false) {
		webtrans = "<strong>Õ¯¬Á Õ“Â:</strong><br/>" + webtrans;
		res.innerHTML += webtrans;
	}
	if(noBaseTrans == false || noWebTrans == false)
	{
		res.innerHTML +="<a href ='http://dict.youdao.com/search?q="+encodeURIComponent(_word)+"&ue=utf8&keyfrom=chrome.extension"+lan+"' target=_blank>µ„ª˜ ≤Èø¥œÍœ∏ Õ“Â</a>";
	}
	if(noBaseTrans && noWebTrans)
	{
		res.innerHTML = "Œ¥’“µΩ”¢∫∫∑≠“Î!";
		res.innerHTML +="<br><a href ='http://www.youdao.com/search?q="+encodeURIComponent(_word)+"&ue=utf8&keyfrom=chrome.extension' target=_blank>≥¢ ‘”√”–µ¿À—À˜</a>";
	}
	retphrase='';
	webtrans = '';
	basetrans = '';
